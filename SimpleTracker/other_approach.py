...
def problem_setup(T, dt, max_velo, start_pos, evader_trajectories,
    ...
    ws = ca.MX(np.ones(T)*np.array([0.8**k for k in range(T)]))
    ...
    if obstacles:
        for obs in obstacles:
            obs_center_np = np.array(obs[0]) # Keep as NumPy array initially
            obs_radius = obs[1]
            repeated_obs_center = ca.MX(np.tile(obs_center_np, (T, 1))) 
            diff_to_obs = x - repeated_obs_center 
            dist_sq_to_obs = ca.sumsqr(diff_to_obs) 
    ...
    total_objective = base_objective + obstacle_penalty_weight * total_obstacle_penalty 
    ...
    # Start position constraint - MODIFIED
    # Ensure both sides of the subtraction are column vectors (2x1)
    # x[0, :] is (1,2), so x[0, :].T is (2,1)
    # ca.MX(start_pos) is (2,1)
    start_pos_violation = x[0, :].T - ca.MX(start_pos)
    constraints.append(start_pos_violation)

    # Motion constraint (remains the same as it correctly produces (T-1)x1)
